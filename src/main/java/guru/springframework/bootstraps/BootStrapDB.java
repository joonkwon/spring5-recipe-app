...
public class BootStrapDB implements ApplicationListener<ContextRefreshedEvent> {
	...
	private List<Recipe> getRecipes(){
		...
		recipe.setNotes(new Notes(recipe, "This recipe makes 12 large or 16 small wedges—the cookies are rich, so the small wedges are not too small if you want to have a greater yield."));
		...
		recipe1.setNotes(new Notes(recipe1,"When I was a kid, I always looked forward to days when my mother made fried rice for dinner. Like most Asian families, we ate regular plain rice almost every day with dinner, so any time my mother felt inspired to change up our dinner routine felt like a treat to me!\n" + 
		...
		recipe1.addIngredient(new Ingredient("minced ginger", new BigDecimal(3),  recipe1, pinchUom));
		recipe1.addIngredient(new Ingredient("Chinese five-spice powder", new BigDecimal(1.5), recipe1, teaspoonUom));
		recipe1.addIngredient(new Ingredient("frozen peas, rinsed under warm tap water for a few seconds thawr", new BigDecimal(2/3), recipe1, cupUom));
